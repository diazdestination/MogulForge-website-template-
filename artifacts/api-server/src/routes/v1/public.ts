...
// Hosted referral form page — the URL customers actually receive in emails.
// Loads business info from the JSON API then presents a name/email/phone/notes
// form without requiring any external scripts or build step. Follows the same
// no-store, self-contained HTML pattern as /public/form-page/:slug.
router.get(
  "/public/referral/:token",
  rateLimit({ windowMs: 60_000, max: 30, key: "referral-page" }),
  (req: Request, res: Response): void => {
    const token = String(req.params.token);
    // Tokens are base64url (18 random bytes → 24 chars); reject obviously wrong shapes.
    if (!/^[A-Za-z0-9_-]{10,40}$/.test(token)) {
      res.status(400).send("Invalid link");
      return;
    }
    // The API origin is the same server — use a relative path so this page
    // works regardless of the deployment domain.
...
(function(){
  ...
  function showNotFound(){
    app.innerHTML = '<div class="not-found"><div style="font-size:2.5rem">🔗</div><h1>Link not found</h1><p>This referral link may have expired or is no longer valid.</p></div>';
  }

  function showSuccess(bizName){
    app.innerHTML = '<div class="success-card"><div class="success-icon">🎉</div><h1>Thank you!</h1><p>Your referral has been sent to '+escHtml(bizName)+'. We appreciate you spreading the word!</p></div>';
  }
